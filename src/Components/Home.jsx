...
const Home = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoadingData(true);
      const URL = `https://www.eporner.com/api/v2/video/search/?query=All&per_page=500&page=${query}&thumbsize=big&order=top-monthy&gay=0&lq=1&format=json`;
      try {
      ...
      <div className="container" style={{marginTop: '30px',}}>
        {data.videos.slice(0, 40).map((video, index) => (
          <div key={index} className="video-box">
            ...
            <Link to="#"><p>{video.keywords.split(",")[2]}</p></Link>
          </div>
        ))}
      </div>
      <div className="ul">
